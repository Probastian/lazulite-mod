 ...
 * ({@code platform/ui/specification.md}, UI-FR1-UI-FR7) -- Yarn-mapped
...
public final class DropdownWidget {
    ...
    public int render(DrawContext context, int x, int y, int width, int rowHeight, int mouseX, int mouseY) {
        ...
        context.drawTextWithShadow(textRenderer, selected.label(), x + ROW_PADDING, y + ROW_PADDING, TEXT_COLOR);
        ...
        for (Option option : options) {
            ...
            context.fill(x, rowY, x + width, rowY + rowHeight, BACKGROUND);
            context.drawTextWithShadow(textRenderer, option.label(), x + ROW_PADDING, rowY + ROW_PADDING, TEXT_COLOR);
            if (overRow && option.description() != null && !option.description().isEmpty()) {
                ...
                for (String line : wrapMessage(textRenderer::getWidth, option.description(), maxTextWidth)) {
                    context.drawTextWithShadow(textRenderer, line, x + ROW_PADDING, textY, TEXT_COLOR);
