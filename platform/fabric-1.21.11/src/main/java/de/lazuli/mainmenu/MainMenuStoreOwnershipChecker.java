 ...
 * plan Decision 1) -- {@code fabric-1.21.11} port, byte-for-byte identical to
 * the {@code fabric-26.1}/{@code fabric-26.2} version: this class imports
 * only {@code com.codedisaster.steamworks.*}, never any {@code net.minecraft.*}
 * type, so it needs no Yarn/Mojang-mapping-specific changes across platform
 * modules.
 *
 * <p>The sole {@code com.codedisaster.steamworks.SteamInventory*}-importing
 * class this feature introduces on this module -- follows the same
 * one-gateway-class, fail-closed, never-throw, warn-sink discipline
