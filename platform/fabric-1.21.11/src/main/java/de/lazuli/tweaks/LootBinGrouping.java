 ...
 * -- Yarn (1.21.11) port of {@code fabric-26.1}/{@code fabric-26.2}'s copy
 * of this file. Only the Minecraft-typed section (types/method names) below
 * differs from the Mojmap copies; the pure, Minecraft-type-free section is
 * byte-identical (confirmed by direct comparison).
 *
 * <p>Split into two sections: a Minecraft-type-free "pure logic" section
 * (R6/R7/R12/R16's sub-algorithms, unit tested once in the fabric-26.2
 * module's own test tree per the plan's Test Strategy -- {@code
...
public final class LootBinGrouping {
    ...
    private static Map<Item, ItemGroup> reverseIndex;
    private static List<ItemGroup> tabOrder;

    /** R4: cached, lazily built, invalidated on resource/data reload. */
    public static synchronized Map<Item, ItemGroup> reverseIndex() {
    ...
    public static synchronized List<ItemGroup> tabOrder() {
        if (tabOrder == null) {
            buildReverseIndex();
        }
        return tabOrder;
    ...
    private static void buildReverseIndex() {
        Map<Item, ItemGroup> index = new HashMap<>();
        List<ItemGroup> order = ItemGroups.getGroupsToDisplay();
        for (ItemGroup group : order) {
            for (ItemStack stack : group.getDisplayStacks()) {
                index.putIfAbsent(stack.getItem(), group);
            }
        }
        reverseIndex = index;
        tabOrder = order;
    ...
    public static boolean isPlayerSlot(Slot slot, Inventory playerInventory) {
        return slot.inventory == playerInventory;
    }
    ...
    public static final class EntryKey {
        ...
        public boolean equals(Object obj) {
            return obj instanceof EntryKey other
                    && ItemStack.areItemsAndComponentsEqual(representative, other.representative);
        }
    ...
    public record GroupResult(ItemGroup tab, List<AggregatedEntry> entries) {
    }
    ...
    public static List<GroupResult> computeGroups(List<Slot> slots, Inventory playerInventory,
        ...
        Map<Item, ItemGroup> index = reverseIndex();
        Map<ItemGroup, List<AggregatedEntry>> byTab = new LinkedHashMap<>();
        for (Map.Entry<EntryKey, List<RealSlot>> entry : byKey.entrySet()) {
            Item item = entry.getKey().representative().getItem();
            ItemGroup tab = index.get(item);
            if (tab == null) {
        ...
        for (Map.Entry<ItemGroup, List<AggregatedEntry>> e : byTab.entrySet()) {
            sortEntries(e.getValue(), e.getKey(), sortWithinGroup);
        }

        List<ItemGroup> orderedTabs = new ArrayList<>(byTab.keySet());
    ...
    public static Text displayNameText(ItemStack stack) {
