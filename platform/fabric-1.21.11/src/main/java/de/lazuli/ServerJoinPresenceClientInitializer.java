 ...
 * platform module ({@code fabric-1.21.11}, Yarn-mapped). Obtains the shared
 * {@link SteamworksService}/{@link SteamFriendsGateway} via their hand-offs
 * (never re-initializes Steamworks), loads this feature's config, and --
 * when Steam is available and the feature is enabled -- constructs the
 * session lifecycle/presence scanner, registers this feature's route with
...
public final class ServerJoinPresenceClientInitializer implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        ClientPlayConnectionEvents.JOIN.register((handler, sender, client) -> {
            ...
            HostPortSplit split = HostPortSplit.parse(server.address);
            ...
            String serverName = server.name != null ? server.name : server.address;
            MainMenuJoinHistoryWriteHandoff.ifPublishedServerJoin(
                    new de.lazuli.features.mainmenu.config.MainMenuJoinHistoryConfig.ServerJoinEntry(
                            server.address, serverName, System.currentTimeMillis()));
        });
     ...
     * {@code ServerInfo.address} address string -- kept local rather than
     * reusing {@code ServerAddress} accessors to avoid depending on an
