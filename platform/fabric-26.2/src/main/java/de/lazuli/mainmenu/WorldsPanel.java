...
public final class WorldsPanel {
    ...
    /**
     * FX3.1: "+ Create New World" is only visible while the Worlds tab is the
     * active one.
     *
     * <p>Bug fix (worlds-tab-refresh-on-sync-completion): a download or
     * upload can finish entirely in the background -- e.g. an auto-upload
     * triggered while the player is browsing another tab, or a cloud-only
     * "Download" pill's screen-less restore -- with nothing else forcing this
     * panel's per-world sync/freshness caches to catch up before the player
     * next looks at this tab. Refreshing them here, every time this tab
     * becomes the active one, is the cheapest existing hook that guarantees
     * the row the player is about to see is never stale, without paying
    ...
    public void render(GuiGraphicsExtractor guiGraphics, Font font, int x, int y, int width, int height, int mouseX, int mouseY) {
        ...
        if (summaries.isEmpty() && cloudOnlyWorlds.isEmpty()) {
    ...
    /**
     * Requirement 4: 26.2's real world-launch mechanism, already proven in
