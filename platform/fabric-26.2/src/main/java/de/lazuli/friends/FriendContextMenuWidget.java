...
public final class FriendContextMenuWidget extends AbstractWidget {
    ...
    private boolean isEnabled(int index) {
        ...
        return switch (index) {
            ...
            // Falls back to the friends-sidebar state machine (always false)
            // when that feature is absent/disabled (null reader).
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        ...
        if (index >= 0 && index < LABELS.length && isEnabled(index)) {
            switch (index) {
                ...
                // Hosting's join operation instead of the friends-sidebar
                // no-op (only reachable when isEnabled(3) was true above).
                case 3 -> {
