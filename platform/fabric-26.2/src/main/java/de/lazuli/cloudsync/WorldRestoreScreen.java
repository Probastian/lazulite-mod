 ...
 * (cloud-world-download spec) on Minecraft 26.1 (Mojang-mapped, unobfuscated).
 *
 ...
 * filled rectangle, drawn via {@link GuiGraphicsExtractor} primitives (26.x's
 * renamed/refactored rendering entry point, {@code extractRenderState},
 * replacing the older {@code render(DrawContext,...)} model 1.21.11 still
 * uses -- see {@code .claude/context/minecraft.md}'s Known Cross-Version API
...
public final class WorldRestoreScreen extends Screen {
    ...
    public WorldRestoreScreen(
            ...
            Runnable onReturn) {
        super(Component.literal("Downloading '" + summary.displayName() + "' from Steam Cloud..."));
    ...
    protected void init() {
        addRenderableWidget(Button.builder(Component.literal("Cancel"), button -> onCancel())
                .bounds(width / 2 - 50, height / 2 + 52, 100, 20)
                .build());
    ...
    public void extractRenderState(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float delta) {
        ...
        if (failure != null) {
            guiGraphics.centeredText(Minecraft.getInstance().font,
                    Component.literal("Restore failed: " + RestoreFailureMessages.toPlayerMessage(failure)),
        ...
        if (presenter != null) {
            ...
            guiGraphics.fill(barX, barY, barX + (int) (barWidth * stats.overallFraction()), barY + 10, 0xFF33AA33);

            guiGraphics.centeredText(Minecraft.getInstance().font,
                    Component.literal(stats.percentage() + "%"), width / 2, barY + 14, 0xFFFFFF);
            guiGraphics.centeredText(Minecraft.getInstance().font,
                    Component.literal(stats.currentSizeText() + " / " + stats.totalSizeText()), width / 2, barY + 26, 0xFFFFFF);
            guiGraphics.centeredText(Minecraft.getInstance().font,
