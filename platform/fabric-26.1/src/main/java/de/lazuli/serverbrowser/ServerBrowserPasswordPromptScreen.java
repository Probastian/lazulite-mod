 ...
 * <p>Usage example:
 * <pre>{@code
 * Minecraft.getInstance().setScreenAndShow(new ServerBrowserPasswordPromptScreen(serverBrowserScreen, row));
 * }</pre>
 */
public final class ServerBrowserPasswordPromptScreen extends Screen {
    ...
    @Override
    protected void init() {
        passwordField = new EditBox(minecraft.font, width / 2 - 100, height / 2 - 10, 200, 20, Component.literal("Password"));
        passwordField.setResponder(this::onTextChanged);
        addRenderableWidget(passwordField);

        addRenderableWidget(Button.builder(Component.literal("Join"), button -> onJoin())
     ...
     * separately -- {@link EditBox} has no native masked-input mode. Applies
     * cleanly for the common append/backspace-at-end typing pattern this
     * single-purpose prompt is used for.
     */
    private void onTextChanged(String text) {
        ...
        if (!text.equals(maskedValue)) {
            passwordField.setValue(maskedValue);
            passwordField.setCursorPosition(maskedValue.length());
        }
    }
    ...
    public void extractRenderState(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float delta) {
        ...
        guiGraphics.centeredText(minecraft.font,
                Component.literal("Enter password for " + row.serverName()),
