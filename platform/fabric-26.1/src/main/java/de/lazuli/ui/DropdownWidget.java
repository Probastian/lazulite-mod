 ...
 * ({@code platform/ui/specification.md}, UI-FR1-UI-FR18) -- Mojang-mapped
...
public final class DropdownWidget {
    ...
    public int render(GuiGraphicsExtractor guiGraphics, int x, int y, int width, int rowHeight, int mouseX, int mouseY) {
        ...
        if (label != null && !label.isEmpty()) {
            guiGraphics.text(font, label, x + ROW_PADDING, rowY, TEXT_COLOR);
            rowY += LABEL_HEIGHT;
        ...
        guiGraphics.fill(x, rowY, x + width, rowY + rowHeight, overClosedRow ? BACKGROUND_HOVER : BACKGROUND);
        drawBorder(guiGraphics, x, rowY, width, rowHeight);
        guiGraphics.text(font, selected.label(), x + ROW_PADDING, rowY + ROW_PADDING, TEXT_COLOR);
        ...
        // dropdowns).
        guiGraphics.text(font, ARROW_GLYPH, x + width - ROW_PADDING - font.width(ARROW_GLYPH), rowY + ROW_PADDING, TEXT_COLOR);
        if (overClosedRow && selected.description() != null && !selected.description().isEmpty()) {
            guiGraphics.setTooltipForNextFrame(Component.literal(selected.description()), mouseX, mouseY);
        }
     ...
     * {@code false}.
     */
    public void renderOpenOverlay(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float delta) {
        if (!open) {
            return;
        }
        var font = Minecraft.getInstance().font;
        ...
        for (Option option : options) {
            ...
            guiGraphics.fill(cachedX, rowY, cachedX + cachedWidth, rowY + cachedRowHeight, overRow ? OPTION_BACKGROUND_HOVER : OPTION_BACKGROUND);
            drawBorder(guiGraphics, cachedX, rowY, cachedWidth, cachedRowHeight);
            guiGraphics.text(font, option.label(), cachedX + ROW_PADDING, rowY + ROW_PADDING, TEXT_COLOR);
            if (overRow && option.description() != null && !option.description().isEmpty()) {
                guiGraphics.setTooltipForNextFrame(Component.literal(option.description()), mouseX, mouseY);
            }
